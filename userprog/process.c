...
/* Create a minimal stack by mapping a zeroed page at the top of
   user virtual memory. */
static bool
setup_stack (void **esp, child_t *child ) 
{
  uint8_t *kpage;
  bool success = false;
  int i;
  ...
  kpage = palloc_get_page (PAL_USER | PAL_ZERO); // Get a page from user pool
  if (kpage != NULL) 
    {
  ...
  mySP.byte = *esp; // start at bottom
  for(i = child->argc-1; i >=0; i--){
      mySP.byte -= child->args[i].len+1; //go by len up
      strlcpy(mySP.byte, child->args[i].name,child->args[i].len+1); //copy bytes
      child->args[i].name = mySP.byte;//save the location in stack where the bytes are
                                      // for later use
  }
  //pad bytes to align at word
  i = ((uint32_t)mySP.byte %4);
  mySP.byte -= i;
  ...
  *mySP.word = 0;
  ...
  for(i = child->argc-1; i >=0; i--){
      mySP.word -= 1;
      *mySP.word = (uint32_t *)child->args[i].name;
  }
  *(mySP.word-1) = mySP.word; // argv has &argv[0]
  mySP.word -= 2;
  *mySP.word = child->argc;
  mySP.word -= 1;
  *mySP.word = 0; // The return address is on top
  *esp = mySP.word; // Update esp;
